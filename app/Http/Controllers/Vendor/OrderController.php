...
class OrderController extends Controller
{
    public function orders(Request $request) {
      if ($request->order_number) {
        $orderids = Order::join('orderedproducts', 'orders.id', '=', 'orderedproducts.order_id')->select('orders.id')->groupBy('orders.id')->where('vendor_id', Auth::guard('vendor')->user()->id)->where('orders.unique_id', $request->order_number)->whereIn('orders.approve', [0,1])->get();
        $orderidarr = [];
        ...
        $orderids = Order::join('orderedproducts', 'orders.id', '=', 'orderedproducts.order_id')->select('orders.id')->groupBy('orders.id')->where('vendor_id', Auth::guard('vendor')->user()->id)->whereIn('orders.approve', [0,1])->get();
        $orderidarr = [];
        foreach ($orderids as $key => $orderid) {
          $orderidarr[] = $orderid->id;
        }
